...
describe('generateMetadata', () => {
  ...
  test('returns META_TITLE when set', async () => {
    getRuntimeConfig.mockReturnValue({ META_TITLE: 'Custom Title' });
    const metadata = await generateMetadata();
    expect(metadata.title).toBe('Custom Title');
  });

  test('includes META_DESCRIPTION when set', async () => {
    getRuntimeConfig.mockReturnValue({ META_DESCRIPTION: 'A description' });
    const metadata = await generateMetadata();
  ...
  test('includes META_AUTHOR in authors array', async () => {
    getRuntimeConfig.mockReturnValue({ META_AUTHOR: 'Tim' });
    const metadata = await generateMetadata();
  ...
  test('includes META_KEYWORDS when set', async () => {
    getRuntimeConfig.mockReturnValue({ META_KEYWORDS: 'a, b, c' });
    const metadata = await generateMetadata();
  ...
  test('robots uses META_INDEX_STATUS when set', async () => {
    getRuntimeConfig.mockReturnValue({ META_INDEX_STATUS: 'index' });
    const metadata = await generateMetadata();
  ...
  test('injection: malicious META_TITLE is escaped by Next.js', async () => {
    ...
    getRuntimeConfig.mockReturnValue({ META_TITLE: malicious });
    const metadata = await generateMetadata();
