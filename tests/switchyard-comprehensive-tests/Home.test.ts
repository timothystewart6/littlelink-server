...
/**
 * Test helper with only the specified config keys set
 */
...
test('renders Home component with minimal config', () => {
...
test('renders NAME when config.NAME is set', () => {
  const html = renderToStaticMarkup(React.createElement(Home, { config: { ...baseConfig, NAME: 'Test' } }));
...
test('renders Avatar with size', () => {
  ...
  assert.ok(html.includes('avatar-size-large') || html.includes('width') || html.includes('height'));
});
...
test('renders email alt text when EMAIL_ALT is set', () => {
  const html = renderToStaticMarkup(React.createElement(Home, { config: { ...baseConfig, EMAIL_ALT: 'alt@example.com' } }));
  assert.ok(html.includes('mailto:'));
});
...
test('renders Skoob button when SKOOB is set', () => {
  const html = renderToStaticMarkup(React.createElement(Home, { config: { ...baseConfig, SKOOB: 'https://skoob.jp' } }));
...
test('renders custom buttons when CUSTOM_BUTTON_TEXT is set', () => {
  const html = renderToStaticMarkup(React.createElement(Home, { config: { ...baseConfig, CUSTOM_BUTTON_TEXT: 'Test', CUSTOM_BUTTON_NAME: 'test', CUSTOM_BUTTON_URL: 'https://example.com' } }));
  assert.ok(html.includes('data-order'));
});

test('renders BUTTON_ORDER links', () => {
  const html = renderToStaticMarkup(React.createElement(Home, { config: { ...baseConfig, BUTTON_ORDER: 'github,twitter', CUSTOM_BUTTON_TEXT: 'order' } }));
  assert.ok(html.includes('data-order'));
});

test('renders email with custom text when EMAIL and EMAIL_TEXT are set', () => {
  const html = renderToStaticMarkup(React.createElement(Home, { config: { ...baseConfig, EMAIL: 'test@example.com', EMAIL_TEXT: 'Contact' } }));
